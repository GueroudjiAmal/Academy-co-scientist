...
def _find_abstract_in_text(full_text: str) -> str | None:
    ...
    for m in re.finditer(
        r'(?mi)^(?:\s*)(' + '|'.join(_ABSTRACT_HEADINGS) + r')\s*[:—\-]?\s*$', text
    ):
        start_match = m
        break
    if not start_match:
        ...
        for m in re.finditer(
            ...
            for sm in re.finditer(
                r'(?mi)^(?:\s*)(?:' + '|'.join(_STOP_HEADINGS) + r')\s*[:—\-]?\s*$', text[m.end() :]
            ):
                stop = m.end() + sm.start()
                break
    ...
    stop = None
    for m in re.finditer(
        r'(?mi)^(?:\s*)(?:' + '|'.join(_STOP_HEADINGS) + r')\s*[:—\-]?\s*$', text[start:]
    ):
        stop = start + m.start()
        break
...
def _extract_abstract_from_pdf(path: str) -> tuple[str | None, int]:
