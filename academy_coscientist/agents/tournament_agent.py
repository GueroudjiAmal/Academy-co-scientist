...
class TournamentAgent(Agent):
    ...
    def _normalize_idea(self, idea: Dict[str, Any]) -> Dict[str, Any]:
        ...
        meta = {k: v for k, v in idea.items() if k not in ("title", "name", "description", "text", "score", "confidence")}
    ...
    async def add_hypotheses(self, ideas: List[Dict[str, Any]]) -> List[str]:
        ...
        for idea in ideas or []:
            ...
            ids.append(hid)
        log_action(self.logger, "add_hypotheses", {"n": len(ideas or [])}, {"total": len(self._hyps)})
        return ids

    @action
    async def get_all_hypotheses(self) -> List[Tuple[str, Dict[str, Any]]]:
        out = [(hid, {k: v for k, v in rec.items() if k != "id"}) for hid, rec in self._hyps.items()]
    ...
    async def get_top_hypotheses(self, k: int = 0) -> List[Tuple[str, Dict[str, Any]]]:
        rows = sorted(self._hyps.values(), key=lambda r: (r.get("score", 0.0), r.get("confidence", 0.0)), reverse=True)
        if k and k > 0:
            rows = rows[:k]
    ...
    async def set_scores(self, scores: Dict[str, Dict[str, Any]]) -> None:
        ...
        for hid, payload in (scores or {}).items():
            ...
            try:
                ...
                continue
        log_action(self.logger, "set_scores", {"incoming": len(scores or {})}, {"updated": updated, "missing": missing})
    ...
    async def run_tournament(self) -> None:
        for rec in self._hyps.values():
            ...
            if s is None or math.isnan(s):
                ...
                rec["score"] = float(s)
        log_action(self.logger, "run_tournament", {"op": "normalize_scores"}, {"count": len(self._hyps)})
