    ...
    async def evolve_top(self, k: int = 10) -> None:
        ...
        for (hid, elo, idea) in top_list:
            ...
            combined = {
                ...
                "feasibility_risks": sum([c.get("feasibility_risks", []) for c in all_critiques], []),
                "suggested_improvements": sum([c.get("suggested_improvements", []) for c in all_critiques], []),
        ...
        log_action(self.logger, "evolve_top", {"k": k}, {"num_evolved": len(evolved_ids), "evolved_ids": evolved_ids})
