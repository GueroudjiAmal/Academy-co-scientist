...
class SupervisorAgent(Agent):
    ...
    def __init__(self) -> None:
        ...
        self._review_k: Optional[int] = None
    ...
    @action
    async def set_counts(self, hypotheses: int, review_k: Optional[int] = None) -> None:
        self._last_n_ideas = int(hypotheses)
        self._review_k = int(review_k) if review_k is not None else None
        log_action(self.logger, "set_counts",
                   {"hypotheses": self._last_n_ideas, "review_k": self._review_k}, {"ok": True})
    ...
    async def _review_with_fallback(self, reviewer, tournament) -> None:
        ...
        if ok:
            log_action(self.logger, "review_dispatch", {"reviewer": str(reviewer), "method": "review_all"}, {"ok": True})
            return

        k = self._review_k if self._review_k is not None else self._last_n_ideas if self._last_n_ideas > 0 else None
        if k is not None:
        ...
        if ok:
            log_action(self.logger, "review_dispatch",
                       {"reviewer": str(reviewer), "method": "review_top", "k": k}, {"ok": True})
            return

        log_action(self.logger, "review_dispatch",
                   {"reviewer": str(reviewer)}, {"warning": "No usable review_* found"})
    ...
    async def _collect_reviews(self) -> Dict[str, Dict[str, Any]]:
        ...
        log_action(self.logger, "collect_reviews",
                   {"ra_count": len(ra), "rb_count": len(rb)},
                   {"merged_count": len(merged)})
        return merged

    async def _push_reviews_to_tournament(self, merged_reviews: Dict[str, Dict[str, Any]]) -> None:
        payload = {hid: {"score": v.get("score", 0.0), "confidence": v.get("confidence", 0.0), "title": v.get("title")}
        ...
        for m in methods:
            ...
            if ok:
                log_action(self.logger, "tournament_score_inject", {"method": m, "count": len(payload)}, {"ok": True})
                return
        log_action(self.logger, "tournament_score_inject",
    ...
    async def run_full_cycle(self) -> str:
        commentary = await utils_llm.agent_self_commentary(
            "SupervisorAgent", {"phase": "start", "topic": self._topic},
        ...
        if self._last_n_ideas <= 0:
            raise RuntimeError("SupervisorAgent.set_counts(hypotheses=...) must be called with a positive value.")
