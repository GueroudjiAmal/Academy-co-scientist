...
def _discover_local_exchange_class() -> Optional[Type[Any]]:
    try:
...
async def _robust_launch_vstore_with_manager(manager, path: str, logger) -> Any:
    ...
    if hasattr(vstore, "set_embeddings_dir"):
        func = getattr(vstore, "set_embeddings_dir")
        if inspect.iscoroutinefunction(func):
            await func(path)
        else:
            func(path)
    elif hasattr(vstore, "set_path"):
        func = getattr(vstore, "set_path")
        if inspect.iscoroutinefunction(func):
            await func(path)
        else:
            func(path)
    elif hasattr(vstore, "embeddings_dir"):
        setattr(vstore, "embeddings_dir", path)
    else:
        logger.warning("VectorStoreAgent has no setter for embeddings dir; proceeded without setting path",
                       extra={"path": path})
    return vstore
...
async def _run_with_manager(topic: str, n_hypotheses: int, review_k: Optional[int], logger) -> str:
    ...
    logger.debug("using exchange", extra={"class": f"{ExchangeCls.__module__}.{ExchangeCls.__name__}"})
...
async def _run_direct_mode(topic: str, n_hypotheses: int, review_k: Optional[int], logger) -> str:
...
async def run_pipeline(topic: str, n_hypotheses: int, review_k: Optional[int]) -> str:
    ...
    logger.info("launch_params", extra={
        "topic": topic,
    ...
    abstracts_dir = get_path("abstracts_cache_dir", default=os.path.join(embeddings_dir, "abstracts"))
...
def main() -> None:
    ...
    parser.add_argument("--hypotheses-count", type=int, required=True, help="Number of hypotheses to generate.")
    parser.add_argument("--review-k", type=int, required=False,
